...
def parse_daily_tools(content):
    ...
    It handles multiple date header formats and numbered lists.
    ...
    # Pattern 1: "## YYYY-MM-DD - Daily AI Tools Digest"
    # Pattern 2: "## AI Tools and Apps of the Day: Month DD, YYYY"
    # Pattern 3: "## AI Tools and Apps of the Day: Month DD, YYYY" (alternative format)
    ...
    # Second, try the legacy format: "## AI Tools and Apps of the Day: Month DD, YYYY"
    legacy_sections = re.findall(r'## AI Tools and Apps of the Day: (.*?)\n(.*?)(?=\n##|\Z)', content, re.S)
    
    for date_str, section_content in legacy_sections:
        # Parse date string to ISO format
        try:
            # Handle formats like "June 20, 2025"
            date = datetime.strptime(date_str.strip(), '%B %d, %Y').isoformat()
        except ValueError:
            # Skip invalid date formats
            continue

        # Extract tool information using regex pattern for legacy format
        # Pattern matches: "1. Tool Name – [URL](URL) – Description"
        tool_items = re.findall(r'\d+\.\s(.*?)\s–\s\[(.*?)\]\((.*?)\)\s–\s(.*?)\n', section_content)
        ...
        if tools:
            daily_updates.append({
                "date": date,
                "type": "tools",
                "title": f"AI Tools of the Day: {date_str.strip()}",
                "data": tools
            })
